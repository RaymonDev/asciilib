//pedestrian humanoid prefabs, archetypes, and articulated gait simulations
...
export class PedestrianEntity extends Entity {
  ...
  render(camera, blitter, scene, planar, horizon) {
    ...
    const boxes = [
      { minX: -0.045, maxX: 0.045, minY: -0.045 + hipSway * 0.5, maxY: 0.045 + hipSway * 0.5, minZ: headZBase, maxZ: headZTop, type: 'head' },
      { minX: -0.05, maxX: 0.05, minY: -0.075 + hipSway, maxY: 0.075 + hipSway, minZ: torsoZBase, maxZ: torsoZTop, type: 'torso' },
      { minX: -0.035 + armSwing, maxX: 0.035 + armSwing, minY: 0.075 + hipSway, maxY: 0.11 + hipSway, minZ: 0.34 + verticalBounce * 0.5, maxZ: 0.70 + verticalBounce, type: 'arm' },
      { minX: -0.035 - armSwing, maxX: 0.035 - armSwing, minY: -0.11 + hipSway, maxY: -0.075 + hipSway, minZ: 0.34 + verticalBounce * 0.5, maxZ: 0.70 + verticalBounce, type: 'arm' },
      { minX: -0.04 + leftThighOffset, maxX: 0.04 + leftThighOffset, minY: 0.01 + hipSway * 0.3, maxY: 0.065 + hipSway * 0.3, minZ: 0.18 + leftFootZLift * 0.5, maxZ: torsoZBase, type: 'thigh' },
      { minX: -0.04 + leftFootOffset, maxX: 0.04 + leftFootOffset, minY: 0.01 + hipSway * 0.3, maxY: 0.065 + hipSway * 0.3, minZ: leftFootZLift, maxZ: 0.18 + leftFootZLift * 0.5, type: 'foot' },
      { minX: -0.04 + rightThighOffset, maxX: 0.04 + rightThighOffset, minY: -0.065 + hipSway * 0.3, maxY: -0.01 + hipSway * 0.3, minZ: 0.18 + rightFootZLift * 0.5, maxZ: torsoZBase, type: 'thigh' },
      { minX: -0.04 + rightFootOffset, maxX: 0.04 + rightFootOffset, minY: -0.065 + hipSway * 0.3, maxY: -0.01 + hipSway * 0.3, minZ: rightFootZLift, maxZ: 0.18 + rightFootZLift * 0.5, type: 'foot' }
    ];
...
export function updatePedestrianFleet(pedestrians, dt, map, player, mapSize = 80) {
        ...
        for (let tx = minTileX; tx <= maxTileX; tx++) {
          if (tx >= 0 && tx < mapSize && ty >= 0 && ty < mapSize) {
            if (map[ty * mapSize + tx] >= 10) {
              ...
              if (pushDist < pedR && pushDist > 1e-4) {
...
export function createPedestrian(options = {}) {
  const archIndex = options.archetypeIndex !== undefined ? options.archetypeIndex : (options.archetype ? PEDESTRIAN_ARCHETYPES.findIndex(a => a.type === options.archetype) : 0);
  const arch = PEDESTRIAN_ARCHETYPES[Math.max(0, archIndex % PEDESTRIAN_ARCHETYPES.length)];

  return new PedestrianEntity({
    ...arch,
    x: options.x || 0.0,
    y: options.y || 0.0,
    ...
    speed: options.speed || arch.speed,
    ...options
  });
}
