...
import { MAP_SIZE, EW_ROADS, NS_ROADS } from '../cityData.js';
  ...
  render(camera, blitter, scene, planar, horizon) {
    ...
    const boxes = (this.type === 'bus') ? [
      ...
      //trunk / rear end
      {
        ...
        minY: -halfW * 0.90, maxY: halfW * 0.90,
        minZ: 0.16, maxZ: this.beltZ * 0.95,
        type: 'trunk'
      },
      //front-left wheel
      {
    ...
    if (this.type === 'taxi') {
      boxes.push({
        minX: -0.22, maxX: 0.22,
        minY: -0.25, maxY: 0.25,
        minZ: this.roofZ, maxZ: this.roofZ + 0.14,
        type: 'taxilight'
      });
    }
...
export function updateVehicleFleet(vehicles, dt, getSignalState) {
  for (let v = 0; v < vehicles.length; v++) {
    ...
    //kinematic turning & displacement
    if (car.isTurning) {
      if (car.speed > 0) {
      ...
      //intersection approach decisions
      if (car.speed > 1.0) {
  ...
  //vehicle vs vehicle obb collision resolution
