...
export function createStreetFurniture(scene) {
  ...
  function isValidSidewalkLocation(x, y) {
    ...
    //clear of intersections & crosswalks
  ...
  function addTree(x, y, seedVal) {
    ...
    const tree = new TreeEntity({ x, y, seed: seedVal });
    trees.push(tree);
    scene.add(tree);
  }
  ...
  function addLamp(baseX, baseY, armDirX, armDirY) {
    ...
    const armLen = 0.38;
    const lamp = new StreetLightEntity({
      x: baseX,
      y: baseY,
  ...
  function addTrafficLight(baseX, baseY, armDirX, armDirY, facingDir, phaseGroup) {
    ...
    const armLen = 0.82;
    const tl = new TrafficLightEntity({
      x: baseX,
      y: baseY,
      headX: baseX + armDirX * armLen,
      headY: baseY + armDirY * armLen,
